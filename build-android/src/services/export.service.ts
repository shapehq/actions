...
export async function exportResult(result: Result): Promise<void> {
  ...
  if (result.manifestFiles.length > 0) {
    ...
    core.info('')
    core.info(`  Output [ ${MANIFEST_ENV_KEY} = ${lastManifest.path} ]`)
    ...
    core.info(`  Output [ ${MANIFEST_LIST_ENV_KEY} = ${manifestPaths} ]`)
  } else {
