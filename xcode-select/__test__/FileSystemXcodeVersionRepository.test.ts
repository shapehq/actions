...
test("It adds build number to Xcode version name when plist value is present", async () => {
  const fileSystem = new MockFileSystem()
  const xcodePath = "/Users/runner/Applications/Xcode 14.3.app"
  fileSystem.dirContents = [xcodePath]
  const semanticVersionParser = new SemanticVersionParser()
  const xcodeVersionParser = new XcodeVersionParser({ semanticVersionParser })
  const plistReader = new MockPlistReader({
    "/Users/runner/Applications/Xcode 14.3.app/Contents/version.plist:ProductBuildVersion": "ABC123"
  })
  const repository = new FileSystemXcodeVersionRepository({
    fileSystem,
    xcodeVersionParser,
  ...
  const xcodeVersions = await repository.getXcodeVersions()
  expect(xcodeVersions[0].name).toContain("Xcode 14.3.0 (ABC123)")
})

test("It omits build number when plist value is missing", async () => {
  const fileSystem = new MockFileSystem()
  const xcodePath = "/Users/runner/Applications/Xcode 14.3.app"
  fileSystem.dirContents = [xcodePath]
  const semanticVersionParser = new SemanticVersionParser()
